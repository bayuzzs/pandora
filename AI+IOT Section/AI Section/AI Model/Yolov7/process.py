...
class YOLOv7VideoGUI:
    def __init__(self, root):
        ...
        self.btn_frame.columnconfigure((0,1,2,3,4,5), weight=1)
        ...
        self.step_btn = tk.Button(self.btn_frame, text="Step Mode", command=self.step_mode)
        self.step_btn.grid(row=0, column=4, padx=5, sticky="ew")
        self.next_step_btn = tk.Button(self.btn_frame, text="Next Step", command=self.next_step)
        self.next_step_btn.grid(row=0, column=5, padx=5, sticky="ew")
    ...
    def detect_raw(self, frame):
        ...
        return pred, original, img

    def step_mode(self):
        if self.cap:
            ret, frame = self.cap.read()
            if ret:
                pred, original, img = self.detect_raw(frame)
                self.step_data = (pred, original.copy(), img)
                self.step_index = 0
                self.next_step()

    def next_step(self):
        if self.step_data:
            pred, original, img = self.step_data
            display = original.copy()
            log_text = []

            if self.step_index == 0:
                # Step 0: Raw frame only
                self.show_frame(display)
                self.log_label.config(text="Step 1: Raw frame")

            elif self.step_index == 1:
                # Step 1: Show blur/gray mask (simulate attention)
                gray = cv2.cvtColor(display, cv2.COLOR_BGR2GRAY)
                heatmap = cv2.applyColorMap(gray, cv2.COLORMAP_JET)
                self.show_frame(heatmap)
                self.log_label.config(text="Step 2: Feature map simulated")

            elif self.step_index == 2:
                # Step 2: Bounding boxes only
                for det in pred:
                    if det is not None and len(det):
                        det[:, :4] = scale_coords(img.shape[2:], det[:, :4], display.shape).round()
                        for *xyxy, conf, cls in reversed(det):
                            xyxy = [int(x.item()) for x in xyxy]
                            cv2.rectangle(display, (xyxy[0], xyxy[1]), (xyxy[2], xyxy[3]), (0, 255, 0), 2)
                self.show_frame(display)
                self.log_label.config(text="Step 3: Bounding boxes only")

            elif self.step_index == 3:
                # Step 3: Full result with labels
                for det in pred:
                    if det is not None and len(det):
                        det[:, :4] = scale_coords(img.shape[2:], det[:, :4], display.shape).round()
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            xyxy = [int(x.item()) for x in xyxy]
                            cv2.rectangle(display, (xyxy[0], xyxy[1]), (xyxy[2], xyxy[3]), (0, 255, 0), 2)
                            cv2.putText(display, label, (xyxy[0], xyxy[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
                            log_text.append(label)
                self.show_frame(display)
                self.log_label.config(text="Step 4: Final labels\n" + "\n".join(log_text))
