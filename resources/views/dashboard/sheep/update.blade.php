...
@section('content')
  <div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      <form action="{{ route('dashboard.sheep.update', ['sheep' => $sheep->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
    ...
    class="pl-10 w-full h-11 rounded-lg border-gray-300 bg-gray-50 shadow-sm focus:border-green-500 focus:ring-2 focus:ring-green-200 focus:ring-opacity-50 focus:bg-white transition-colors duration-200 @error('uid') border-red-500 @enderror"
    placeholder="Masukkan nomor tag RFID" value="{{ old('uid', $sheep->uid) }}" required readonly>
    ...
    placeholder="Masukkan nama domba" value="{{ old('name', $sheep->name) }}" required>
            </div>
            @error('name')
              <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            @enderror
          ...
          <div>
            ...
            <div class="flex space-x-4">
              <label for="gender_male"
                ...
                {{ old('gender', $sheep->gender) == 'male' ? 'checked' : '' }} required>
              ...
              <label for="gender_female"
                ...
                <input type="radio" id="gender_male" name="gender" value="male"
                class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300"
                {{ old('gender', $sheep->gender) == 'female' ? 'checked' : '' }} required>
          ...
          <div>
            ...
            <div class="relative">
              ...
              value="{{ old('birth_date', $sheep->birth_date->format('Y-m-d')) }}" required>
            </div>
            @error('birth_date')
              <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            @enderror
    ...
    required>
    <option value="" disabled {{ old('breed', $sheep->breed) ? '' : 'selected' }}>Pilih jenis domba</option>
    <option value="merino" {{ old('breed', $sheep->breed) == 'merino' ? 'selected' : '' }}>Merino</option>
    <option value="suffolk" {{ old('breed', $sheep->breed) == 'suffolk' ? 'selected' : '' }}>Suffolk</option>
    <option value="dorper" {{ old('breed', $sheep->breed) == 'dorper' ? 'selected' : '' }}>Dorper</option>
    <option value="texel" {{ old('breed', $sheep->breed) == 'texel' ? 'selected' : '' }}>Texel</option>
    <option value="garut" {{ old('breed', $sheep->breed) == 'garut' ? 'selected' : '' }}>Domba Garut</option>
    <option value="ekor_gemuk" {{ old('breed', $sheep->breed) == 'ekor_gemuk' ? 'selected' : '' }}>Domba Ekor Gemuk</option>
    <option value="batur" {{ old('breed', $sheep->breed) == 'batur' ? 'selected' : '' }}>Domba Batur</option>
    <option value="lainnya" {{ old('breed', $sheep->breed) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
</select>
            </div>
            @error('breed')
              <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
    ...
    placeholder="Masukkan berat domba" value="{{ old('weight', $sheep->weight) }}" required>
            </div>
            @error('weight')
              <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            @enderror
          ...
          <div>
            ...
            <div class="relative">
              ...
              required>
              <option value="" disabled {{ old('health_status', $sheep->health_status) ? '' : 'selected' }}>Pilih status kesehatan</option>
              <option value="Sehat" {{ old('health_status', $sheep->health_status) == 'Sehat' ? 'selected' : '' }}>Sehat</option>
              <option value="Sakit" {{ old('health_status', $sheep->health_status) == 'Sakit' ? 'selected' : '' }}>Sakit</option>
              <option value="Pemulihan" {{ old('health_status', $sheep->health_status) == 'Pemulihan' ? 'selected' : '' }}>Dalam Pemulihan</option>
              <option value="Karantina" {{ old('health_status', $sheep->health_status) == 'Karantina' ? 'selected' : '' }}>Karantina</option>
          </select>
            </div>
            @error('health_status')
              <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            ...
            <div class="relative">
              ...
              <select id="pen_id" name="pen_id"
                ...
                @foreach (\App\Models\Pen::all() as $pen)
                  <option value="{{ $pen->id }}" {{ old('pen_id', $sheep->pen_id) == $pen->id ? 'selected' : '' }}>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="date" id="last_check_date" name="last_check_date"
                ...
                value="{{ old('birth_date', $sheep->last_check_date->format('Y-m-d')) }}">
            </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="date" id="last_vaccination_date" name="last_vaccination_date"
                ...
                value="{{ old('birth_date', $sheep->last_vaccination_date->format('Y-m-d')) }}">
            </div>
